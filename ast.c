...
/* Bandera de revisita para asinaciones */
int cont_revisit = 0; // 1: contiene revisita
...
void set_loop_counter(AST_Node *node){
	...
	if( strcmp(incr_node->entry->st_name, assign_node->entry->st_name) ){
		fprintf(stderr, "La variable usada en la inicializacion y el incremento del for de la linea %d no son la misma!\n", lineno);
		exit(1);
	}	
...
int expression_data_type(AST_Node *node){
	...
	switch(node->type){
		...
		default: /* wrong choice case */
			fprintf(stderr, "Tipo de nodo no reconocido!\n");
			exit(1);
	}
}
...
void ast_print_node(AST_Node *node){
	...
	switch(node->type){
		...
		case DECLARATIONS:
			...
			printf("N Declarations con %d declarations\n", temp_declarations->declaration_count);
			break;
		case DECL_NODE:
			temp_decl = (struct AST_Node_Decl *) node;
			printf("N Declaration de data_type %d para %d names\n",
				temp_decl->data_type, temp_decl->names_count);
			break;
		case CONST_NODE:
			temp_const = (struct AST_Node_Const *) node;
			printf("N Constant Node de const-type %d con valores ", temp_const->const_type);
		...
		case STATEMENTS:
			...
			printf("N Statements con %d statements\n", temp_statements->statement_count);
			break;
		case IF_NODE:
			temp_if = (struct AST_Node_If *) node;
			printf("N If Node con %d elseifs y ", temp_if->elseif_count);
		...
		case FOR_NODE:
			...
			printf("N For con un contador de %s\n", temp_for->counter->st_name);
			break;
		case WHILE_NODE:
			printf("N While\n");
			break;
		...
		case RET_TYPE:
			...
			printf("N Return tipo %d y ", temp_ret_type->ret_type);
			if(temp_ret_type->pointer){
				printf("es un apuntador\n");
			}
			else{
				printf("no es un apuntador\n");
			}
			break;
		case DECL_PARAMS:
			temp_decl_params = (struct AST_Node_Decl_Params *) node;
			printf("N Function declaration parameters de %d parametros\n", temp_decl_params->num_of_pars);
			break;
		case RETURN_NODE:
			temp_return = (struct AST_Node_Return *) node;
			printf("N Return de ret_type %d\n", temp_return->ret_type);
			break;
		default: /* wrong choice case */
			fprintf(stderr, "Error: Nodo no reconocido!\n");
			exit(1);
	}
}
